...
Use these checks before you install a desktop package or deploy an OCI image. Replace `v0.1.0-alpha.0` with the release you want to verify.
...
```sh
mkdir codevault-v0.1.0-alpha.0
cd codevault-v0.1.0-alpha.0
gh release download v0.1.0-alpha.0 --repo CodeVault-LLC/security
```
...
```sh
gh attestation verify ./CodeVault-Security-0.1.0-alpha.0.AppImage \
...
```sh
bun run release:verify -- /absolute/path/to/codevault-v0.1.0-alpha.0
```
