  ...
  it("redacts confidential values recursively without hiding safe identifiers", () => {
    expect(
      redactAuditPayload({
        ...
        subject: "Pre-auth RCE in unreleased product",
        bodyMarkdown: "private reproduction steps",
        manualFields: { reproduction: "run the exploit" },
        manifest: { attachments: [{ filename: "zero-day-poc.zip" }] },
