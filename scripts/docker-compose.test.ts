...
test("the production stack exposes only the loopback API and uses hardened application containers", () => {
  const composeFile = fileURLToPath(
  ...
  for (const name of ["server", "worker", "media-worker"]) {
    ...
    expect(service?.cap_drop, `${name} must drop Linux capabilities`).toContain(
      "ALL",
    );
    expect(
